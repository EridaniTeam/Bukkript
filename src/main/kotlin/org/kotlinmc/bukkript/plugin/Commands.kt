...
import org.kotlinmc.bukkript.plugin.manager.ScriptManagerImpl.Companion.MINIMUM_MODIFY_TIME_TO_RECOMPILE_SECONDS
import org.bukkit.ChatColor
import org.kotlinmc.bukkit.dsl.command.*
import org.kotlinmc.bukkit.dsl.command.arguments.enum
import org.kotlinmc.bukkit.dsl.command.arguments.string
            ...
            command("setlevel") {
                ...
                executor {
                    ...
                    val logLevel = enum<LogLevel>(
                        1,
                        argMissing = textOf("$BUKKRIPT_PREFIX &cPlease, inform the Log Level &a${
                            LogLevel.values().toList()
                        }&c.".translateColor()),
                        notFound = textOf("$BUKKRIPT_PREFIX &cThe Log Level specified was not found, try one of this &a${
                            LogLevel.values().toList()
                        }&c.".translateColor())
                    )
