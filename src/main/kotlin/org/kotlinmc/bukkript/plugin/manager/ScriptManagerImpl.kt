...
import org.kotlinmc.bukkit.architecture.lifecycle.extensions.pluginCoroutineScope
...
class ScriptManagerImpl(
    ...
    override fun compile(scriptName: String): Job {
        ...
        ) throw ScriptInvalidStateException("The script is currently is unloaded or loaded and could not compile again.",
            scriptState::class.java.simpleName)
        ...
        return pluginCoroutineScope.launch(Dispatchers.Default) {
            ...
            val compiled = if (cachedScript != null && cachedScript.isValid) {
                ...
                val description = compiler.retrieveDescriptor(scriptFile)
                    ?: throw RetrieveScriptDefinitionException("Could not retrieve the script $scriptName informations.",
                        scriptName)

                logger.logScript(scriptName,
                    LogLevel.INFO,
                    "Retrieved descriptor: version=${description.version}, author=${description.author}, log level=${description.logLevel}.")
    ...
    override fun load(scriptName: String) {
        val state = scripts[scriptName]
            ?: throw ScriptNotFoundException("Could not load the script $scriptName because it was not found.",
                scriptName)
        val unloaded = (state as? ScriptState.Unloaded)
            ?: throw ScriptInvalidStateException("Could not load the script because the current state of the script is not unloaded.",
                scriptName)
    ...
    override fun unload(scriptName: String) {
        val script = scripts[scriptName]
            ?: throw ScriptNotFoundException("Could not unload the script $scriptName bacause was not found.",
                scriptName)
        val loaded = script as? ScriptState.Loaded
            ?: throw ScriptInvalidStateException("Could not unload the script $scriptName because it is not loaded.",
                scriptName)
    ...
    override fun reload(scriptName: String) {
        val script = scripts[scriptName]
            ?: throw ScriptNotFoundException("Could not reload the script $scriptName because it was not found.",
                scriptName)
        val loaded = script as? ScriptState.Loaded
            ?: throw ScriptInvalidStateException("Could not reload the script $scriptName because it is not loaded.",
                scriptName)
        ...
        val unloaded = scripts[scriptName] as? ScriptState.Unloaded
            ?: throw ScriptInvalidStateException("Could not complete reload the script $scriptName by loading it because was not found the script into the Unloaded state.",
                scriptName)
    ...
    override fun recompile(scriptName: String) {
        val state = scripts[scriptName]
            ?: throw ScriptNotFoundException("Could not recompile the script $scriptName because it was not found.",
                scriptName)
    ...
    override fun lockLog(player: Player, scriptName: String) {
        scripts[scriptName]
            ?: throw ScriptNotFoundException("Could not lock the log into the chat because the script $scriptName because is not found.",
                scriptName)
    ...
    override fun updateLogLevel(scriptName: String, logLevel: LogLevel) {
        scripts[scriptName]
            ?: throw ScriptNotFoundException("Could not update the log level from the script $scriptName because it was not found.",
                scriptName)

        val script = scripts[scriptName] as? ScriptState.Loaded
            ?: throw ScriptInvalidStateException("Could not update the log level from the script $scriptName because it is not loaded.",
                scriptName)
    ...
    private fun discoveryScript(scriptName: String) {
        ...
        if (scriptFile.exists()) {
            ...
            throw ScriptFileDoesNotExistException("Could not find the script file by the name $scriptName!",
                scriptName,
                scriptFile)
        }
    }
